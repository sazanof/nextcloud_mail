	...
	methods: {
		...
		async onAddCloudAttachment() {
			...
			try {
				...
				this.emitNewAttachments(paths.map(function(name, i) {
					const _cloudFile = {
						fileName: name,
					...
					const _toAttachmentData = {
						finished: true,
						imageBlobURL: th.generatePreview(_cloudFile),
						total: filesFromCloud[i].size,
						sizeString: th.formatBytes(filesFromCloud[i].size),
					...
					th.attachments.push(Object.assign(_toAttachmentData, _cloudFile))
